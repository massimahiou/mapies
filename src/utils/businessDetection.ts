...
const QUEBEC_BUSINESSES: Record<string, { category: string; confidence: number }> = {
  // Grocery / Supermarket
  ...
  'jean-coutu': { category: 'pharmacy', confidence: 95 },
  'jc': { category: 'pharmacy', confidence: 95 }, // Jean-Coutu code
...
export function detectBusinessType(name: string, address?: string, hasSmartGrouping: boolean = true): BusinessMatch {
  ...
  for (const { pattern, category, confidence } of obviousPatterns) {
    if (fullText.includes(pattern)) {
      const categoryObj = BUSINESS_CATEGORIES.find(cat => cat.id === category)!
      return {
  ...
  for (const [businessName, data] of Object.entries(QUEBEC_BUSINESSES)) {
    if (normalizedName.includes(businessName) || businessName.includes(normalizedName)) {
      const category = BUSINESS_CATEGORIES.find(cat => cat.id === data.category)!
      return {
        category,
  ...
  let bestMatch: { business: string; similarity: number; data: any } | null = null
  ...
  if (bestMatch && bestMatch.similarity > 0.7) {
    const category = BUSINESS_CATEGORIES.find(cat => cat.id === bestMatch!.data.category)!
    return {
      category,
  ...
  for (const [businessName, data] of Object.entries(QUEBEC_BUSINESSES)) {
    ...
    for (const word of words) {
      if (word.length > 3 && normalizedName.includes(word)) {
        ...
        return {
          category,
          confidence: Math.round(data.confidence * 0.8), // Reduce confidence for partial match
          matchedTerm: word
        }
      }
    }
